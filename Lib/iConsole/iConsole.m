...
- (void)setConsoleText
{
    ...
    [str addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:[text rangeOfString:text]];
    
    NSUInteger index = 0;
    
    for (int i = 0;i<_log.count;i++)
    {
        NSString *message = [_log objectAtIndex:i];
        
        NSString *subText = [text substringFromIndex:index];
        
        NSRange range = [subText rangeOfString:message];

        
        if ([message hasPrefix:[@"> " stringByAppendingString:LOG_LEVEL_INFO]])
        {
            [str addAttribute:NSForegroundColorAttributeName value:self.infoColor range:NSMakeRange(index+range.location, range.length)];
            
        }
        else if ([message hasPrefix:[@"> " stringByAppendingString:LOG_LEVEL_WARNING]])
        {
            [str addAttribute:NSForegroundColorAttributeName value:self.warningColor range:NSMakeRange(index+range.location, range.length)];
        }
        else if ([message hasPrefix:[@"> " stringByAppendingString:LOG_LEVEL_ERROR]])
        {
            [str addAttribute:NSForegroundColorAttributeName value:self.errorColor range:NSMakeRange(index+range.location, range.length)];
        }
        else if ([message hasPrefix:[@"> " stringByAppendingString:LOG_LEVEL_CRASH]])
        {
            [str addAttribute:NSForegroundColorAttributeName value:self.crashColor range:NSMakeRange(index+range.location, range.length)];
        }
        else
        {
            [str addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(index+range.location, range.length)];
        }
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
	if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]))
	{
        ...
        self.infoString = @"iConsole: Copyright © 2015 Haier Design";
        ...
        self.crashColor = [UIColor orangeColor];
        self.infoColor = [UIColor greenColor];
        self.warningColor = [UIColor yellowColor];
...
+ (void)info:(NSString *)format args:(va_list)argList
{
    ...
    {
        [self log:[NSString stringWithFormat:@"%@: %@",LOG_LEVEL_INFO,format] args:argList];
    }
}
...
+ (void)warn:(NSString *)format args:(va_list)argList
{
    ...
    {
        [self log:[NSString stringWithFormat:@"%@: %@",LOG_LEVEL_WARNING,format]  args:argList];
    }
}
...
+ (void)error:(NSString *)format args:(va_list)argList
{
    ...
    {
        [self log:[NSString stringWithFormat:@"%@: %@",LOG_LEVEL_ERROR,format]  args:argList];
    }
}
...
+ (void)crash:(NSString *)format args:(va_list)argList
{
    ...
    {
        [self log:[NSString stringWithFormat:@"%@: %@",LOG_LEVEL_CRASH,format]  args:argList];
    }
}
